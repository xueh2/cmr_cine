...
class NHLBIResUnetInputBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, use_dropout=False, p=0.5, H=256, W=256, verbose=True):
        ...
        if verbose:
            print("    NHLBIResUnetInputBlock : input size (%d, %d, %d), output size (%d, %d, %d) --> (%d, %d, %d)" % (inplanes, H, W, planes, H_conv1, W_conv1, planes, H_conv2, W_conv2))
...
class NHLBIResUnetBasicBlock(nn.Module):

    def __init__(self, inplanes, planes, stride=1, use_dropout=False, p=0.5, H=256, W=256, verbose=True):
        super(NHLBIResUnetBasicBlock, self).__init__()
        ...
        if verbose:
            print("        NHLBIResUnetBasicBlock : input size (%d, %d, %d), output size (%d, %d, %d) --> (%d, %d, %d)" % (inplanes, H, W, planes, H_conv1, W_conv1, planes, H_conv2, W_conv2))
...
class NHLBIResUnet_UpSample(nn.Module):
    def __init__(self, block, layers, in_ch, out_ch, bilinear=True, stride=1, use_dropout=False, p=0.5, H=256, W=256, verbose=True):
        ...
        if verbose:
            print("        NHLBIResUnet_UpSample : input size (%d, %d, %d), upsampled size (%d, %d, %d)" % (in_ch, H, W, in_ch, H_layer_in, W_layer_in))
...
class NHLBIResUnet(nn.Module):
    ...
    def __init__(self, block, F0, inplanes, layers, layers_planes, use_dropout=False, p=0.5, bilinear=True, H=224, W=224, num_classes=10, verbose=True):
        ...
        - Input:
        block: class name for basic building block, e.g. NHLBIResNetBasicBlock
        ...
        super(NHLBIResUnet, self).__init__()
        ...
        if verbose:
            print("NHLBIResUnet : F0=%d, inplanes=%d" % (F0, inplanes))
            print("--" * 30)

        # --------------------------------------------------------------------
        # start up layer
        # --------------------------------------------------------------------
        self.input_layer = NHLBIResUnetInputBlock(F0, inplanes, stride=1, use_dropout=use_dropout, p=p, H=H, W=W, verbose=verbose)
        ...
        for l in range(len(layers)):
            ...
            if verbose:
                print("    NHLBIResUnet, down layer %d:" % l)
        ...
        # --------------------------------------------------------------------
        if verbose:
            print("    NHLBIResUnet, bridge layer (%d, %d, %d) --> (%d, %d, %d)" % (self.layer_planes[l], H_layer, W_layer, 
        ...
        for l in range(len(layers)):
            ...
            if verbose:
                print("    NHLBIResUnet, up layer %d:" % l)
        ...
        if verbose:
                print("    NHLBIResUnet, up layer %d:" % (l+1))
    ...
    def _make_down_layer(self, block, layers, inplanes, planes, stride=1, H_layer=64, W_layer=64, verbose=True):
        ...
        if verbose:
            print("        NHLBIResUnet, down layer (%d, %d) -> (%d, %d)" % (H_layer, W_layer, H_layer_in, W_layer_in))
    ...
    def _make_up_layer(self, block, layers, inplanes, planes, stride=1, H_layer=64, W_layer=64, verbose=True):
        layer = NHLBIResUnet_UpSample(block, layers, inplanes, planes, self.bilinear, stride=stride, use_dropout=self.use_dropout, p=self.dropout_p, H=H_layer, W=W_layer, verbose=verbose)
...
def NHLBIResUnet18(F0=3, inplanes=64, layers=[1,1,1], layers_planes=[128,256,512], use_dropout=False, p=0.5, H=224, W=224, C=2, verbose=True):
    """Constructs a NHLBIResUnet model.
    """
    model = NHLBIResUnet(NHLBIResUnetBasicBlock, F0, inplanes, layers, layers_planes, use_dropout=use_dropout, p=p, H=H, W=W, num_classes=C, verbose=verbose)
    return model
